...
eXperience DHTML coolMenus - Get it atwww.dhtmlcentral.com
Version 3.02
...
Debugging function
********/
function debug(txt,ev){try{if(mDebugging==2) self.status=txt; else alert(txt); if(ev) eval(ev); return false}catch(er) {
...
try{
if(!this.img1){if(bw.ns4){this.ref.write(text);
...
function cm_moveY(y){try{this.y=y; this.css.top=y}catch(er){}}
function cm_moveIt(x,y){try{this.x=x; this.y=y; this.css.left=this.x;this.css.top=this.y}catch(er){}}
function cm_showIt(){try{this.css.visibility="visible"; this.vis=1}catch(er){}};
function cm_hideIt(){try{this.css.visibility="hidden"; this.vis=0}catch(er){}}
...
function cm_filterIt(f){try{if(this.evnt.filters[0]) this.evnt.filters[0].Stop(); else this.css.filter=f; this.evnt.filters[0].Apply(); this.showIt(); this.evnt.filters[0].Play();}catch(er){}}
...
function cm_clipTo(t,r,b,l,w){try{if(bw.ns4){this.css.clip.top=t;this.css.clip.right=r; this.css.clip.bottom=b;this.css.clip.left=l
}else{this.css.clip="rect("+t+","+r+","+b+","+l+")"; if(w){this.css.width=r; this.css.height=b}}; this.width=r; this.height=b}catch(er){}}
...
function cm_NS6_createElement(st,inn){try{el=document.createElement("DIV"); if(st) el.setAttribute("style",st); if(inn) el.innerHTML=inn; document.body.appendChild(el); return el}catch(er){}}
/******
Making code
*******/
...
function cm_check_error(e){try{e=e.toLowerCase(); if(e.indexOf("access")>-1||e.indexOf("permission")>-1){coolFrameError=1; return true;}else return false}catch(er){}}
/*******
Make styles
*******/
...
function cm_unloaded(ev){try{coolFName=eval(coolFName); if(!coolFName) return; if(document.layers && ev.target.name!=coolFName.frame) return; else coolFName.nsload=0; coolFName.loaded=0; if(!document.layers) setTimeout("cm_checkLoaded()",200)}catch(er){}}
/******
Make menu object
*******/
